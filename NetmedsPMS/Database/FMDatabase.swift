...
class FMDBDatabase {
    ...
    static func querySpecificColumns(columnNames: [String], on table:String, where conditions:[WhereCondition]? = nil, completion:ResultCompletion){
        var columnNamesStatemnet = ""
        for columnName in columnNames{
            columnNamesStatemnet += "\(columnName),"
        }
        if let lastCharacter = columnNamesStatemnet.last{
            if lastCharacter == ","{
                columnNamesStatemnet.removeLast()
            }
        }
        var sqlStatement = "SELECT \(columnNamesStatemnet) FROM \(table)"
